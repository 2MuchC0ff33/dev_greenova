...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "Finding obligations with recurring frequencies to normalize..."
        )

        # Get all obligations with a recurring frequency
        obligations = Obligation.objects.filter(
            recurring_obligation=True, recurring_frequency__isnull=False
        ...
        with transaction.atomic():
            ...
            for obligation in obligations:
                ...
                if original.lower() != normalized:
                    self.stdout.write(f"  • {obligation.obligation_number}: '{
                        original}' → '{normalized}'")
                    changes += 1
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(f"Successfully normalized {
                changes} frequencies (skipped {skipped} already normalized)"))
